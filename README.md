...
当前 Web 灰盒控制为：`WASD/方向键` 移动、`F` 交互、`Space` 纵跃、`J` 轻击、`K` 重击、`Shift` 守势、`C` 闪身、`1/2` 切换檐守/翻花，倒地后按 `R` 从当前 Bootstrap 安全点重试。它已经能直观看到《斗战神》第一指导的斜向纵深、深度排序、遮挡层次与大型 Boss 尺度，并连续操作全部 7 Beat：任务推进、两段普通战斗、调查/调校、回程捷径、四季 Boss 换相、双结算选择和返回沈砚均已接通。它仍不是可连续首玩 1 小时的 Demo；当前自动玩家验证的是功能纵切而非正式时长，真实 60 分钟节奏/盲测、奖励持久事务、正式鼠标/手柄动作适配、Boss 门前及跨 Beat 多安全点快照仍是缺项。
...
| 可玩纵切 | In Progress（7/7 Beat 功能闭环 + 可操作战斗/调查/Boss/结算灰盒） | F1 唯一流程、5 Cell/7 Beat 机器合同；代码绘制的 2.5D 斜向全景雨夜场景；雨渡交互、沈砚双架势训练、失败重试、三敌读招清场、路线选择、三证据工灯调查、双调校、回程战/捷径、四季 Boss 四阶段、双结算和返沈砚；资源恢复、任务选择和已提交进度保持 | 真实 60 分钟内容密度/盲测、奖励 Profile 事务、复杂 AI/掉落、真实 5 Cell、多安全点和正式资产 |
| 内容工具 | In Progress（Bootstrap） | 9 类模板注册表、工作台范围、F1 纵切、13 项场景交互、2 项战斗任务触发器、3 项敌群结果、2 项敌群激活、4 项 Boss 阶段、2 项结算奖励映射及 5 实体/17 能力战斗包的 JSON/Schema/确定性 C++ 生成 | 可用 Workbench、通用 ContentCore/baker、迁移、资源预览与错误定位 UI |
