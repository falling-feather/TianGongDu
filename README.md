...
- 已实现确定性 `IQuestRuntime`、目标图快照/事件/校验和与内容驱动的场景交互解析器；玩家现在可按 `F` 完成雨渡出生段的“验看路引 / 抵达渡口门”并推进到第二 Beat。后续训练、工灯调查、路线选择、Boss 四阶段、双结算与奖励仍缺失，但均保留在既有 7 Beat 合同和 Reserved 接口边界内。
...
当前 Web 灰盒控制为：`WASD/方向键` 移动、`F` 交互、`Space` 纵跃、`J` 轻击、`K` 重击、`Shift` 守势、`C` 闪身、`1/2` 切换檐守/翻花，倒地后按 `R` 从当前 Bootstrap 安全点重试。它已经能直观看到《斗战神》第一指导的斜向纵深、深度排序与遮挡层次，以及任务推进、姿态资源、命中/破势、资源恢复和失败重试反馈，但还不是可连续游玩 1 小时的 Demo；后 6 Beat、正式鼠标/手柄动作适配与多安全点快照仍是缺项。
...
| 可玩纵切 | In Progress（首段任务 + 可操作战斗灰盒） | F1 唯一流程、5 Cell/7 Beat 机器合同；代码绘制的 2.5D 斜向全景雨夜场景；移动/纵跃/场景交互、首段两目标推进；双姿态战斗、三敌追击/归位、资源恢复与失败重试 | 后 6 Beat 的 NPC 训练/工灯调查/选择与转场、复杂 AI/掉落、真实 5 Cell、Boss 四阶段、双结算和 60 分钟盲测 |
| 内容工具 | In Progress（Bootstrap） | 9 类模板注册表、工作台范围、F1 纵切、首批场景交互及 4 实体/9 能力战斗包的 JSON/Schema/确定性 C++ 生成 | 可用 Workbench、通用 ContentCore/baker、迁移、资源预览与错误定位 UI |
