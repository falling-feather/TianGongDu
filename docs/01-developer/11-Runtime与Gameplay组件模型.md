...
`DeterministicQuestUiProjectionProducer` 只读取当前 `IQuestRuntime`、移动安全点、Combat Actor 快照和已验证的低频结果信号，输出固定容量 `QuestUiProjectionSnapshot`。快照包含单调 projection sequence、Quest 权威 checksum、cue/Beat/Objective、安全点、唯一 pending Objective、已提交选择、已完成 Objective、排序后的 Active/Defeated hostile、两个各自拥有 ID/Objective/status/reason 的结果槽、稳定 attempt-time classification 与整体 checksum。Actor 身份由 Definition 决定：player key 只能携带 player faction，Encounter/Boss 授权的 hostile key 只能携带 hostile faction，未知、重叠或阵营伪装都失败关闭。attempt-time classification 也按 source 使用固定白名单，choice 不可冒充 combat proof、combat feedback 不可冒充 craft decision；`ignored_repeat` 唯一合法 reason 是 `selection_already_committed`。当前合同不支持多个同时 Active Objective；发现第二个 Active、未知/重复/非法 Actor、未知或错类型 interaction/trigger/outcome、跨 Beat 引用、选择/完成事实不一致或任一容量溢出时失败关闭，且不会替换上一份有效快照或消费 sequence。
...
通用 Native 目录以 14 个彼此独立的权威快照样本覆盖互斥选择、accepted-positive、accepted-negative override、rejected、trigger-only、`trigger accepted + outcome rejected`、恢复 offer/resume 和两套 choice options；另覆盖 selector 同一 result IDs 在 outcome accepted/rejected 时分别得到 positive/negative，以及定义歧义、结果引用、intent stale、Actor 身份/阵营、source/classification 错配、cue Objective/selector 各 8 项、choice/selected/Actor/retained 的 8/16/16/64 容量和失败不变性。锁定 Clang 19 Release 与 MSVC Release 完整 CTest 均为 18/18，Node 为 50/50，Emscripten/Axmol Web Single Release 可重编。

该状态是 **Bootstrap Implemented / Not Authored In Current F1**。为兼容 pre-1.5 C++ 聚合，`VerticalSliceDefinition.quest_ui_cues` 是尾随可空 span；当前 schema 1.4 生成 Definition 留空，因此旧消费者继续编译，而主动初始化投影 producer 会失败关闭。F1 1.5 JSON/Schema/生成器/生成头、Platform/Web envelope、正式 Presentation renderer、Action 接线、训练安全点阶段重建与浏览器证据分别由后续独立提交完成；本节点没有把 14 个 fixture、prototype flow/autoAdvance 或 F1 Objective ID 写入 Runtime。
