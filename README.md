...
2. `GAME-007` 与 `CONTENT-003`：建立通用工艺/工坊经营规则，并制作一段江南伞作代表流程；真实工艺步骤必须有资料来源和审阅记录。
3. `TOOLS-004/005/006`：补编辑器 CRUD、人物/敌人模板、波次/目标、工艺/订单/工坊面板、二维摆放与启动/重载。
4. `GAME-005`：接入 Actor profile、职责、技能、两波战斗、目标推进与终点，并让工艺结果进入探索或战斗。
5. `GAME-006`：把死亡重试扩展为工艺、工坊、关卡、机关、波次、敌人和战斗状态的聚合恢复。
6. `UI/ART/QA`：补必要 HUD、工艺反馈、攻击提示、地区可辨灰盒、Windows/Web/三浏览器证据和真人试玩。
