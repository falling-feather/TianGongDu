...
《天工渡》是一款浏览器首发、复用同一 C++ 核心扩展到 Windows 等平台的 2.5D 斜向全景动作角色扮演游戏。玩家作为年轻的“执灯渡人”，在东方工艺社会中探索、交往和战斗，让失序的器物、工序、责任与关系重新归位。

长期产品包含地区探索、角色关系、工艺调查、伞器兵击、Boss 归位和选择后果。当前不先扩写长剧情，而是推进 `F1-SYSTEM-DEMO-01`：用一个可编辑、可反复试玩的战斗沙盒证明两件事——战斗值得继续做，工具确实能生产内容。
...
| 编辑器能完整制作关卡吗 | 还不能。波次、目标、二维摆放画布、一键试玩和快速重载仍未完成 |
| Windows 和 Web 是否已证明同一 Demo | 核心与包格式已有共同测试；玩家可见的同包双端 Preview 仍未完成 |

工程底座完成度约为 **50%–55%**；按“设计师能制作、玩家能看见并完整试玩”的系统型 Demo 口径约为 **35%–40%**。这些数字用于排期，不是发布质量分数。

## 系统型 Demo 目标

最终交付是一块可重复调试的战斗沙盒：

1. 在轻量编辑器中摆放玩家、敌人、障碍、互动点、机关和安全点。
2. 配置两到三组敌人波次、触发条件、关卡目标与资源。
3. 保存并导出同一内容包，直接启动 Windows 或 Web 试玩。
4. 玩家使用移动、轻重击、防守、闪避、两种架势和首批数据化技能战斗。
5. 敌人至少形成近战压制、侧击/包抄、远程或支援等可辨职责，并包含精英与机制型 Boss 验证位。
6. 死亡、重试、离开返回和重新加载不产生关卡、机关、敌人或技能状态漂移。
7. 灰盒资源允许保留，但不同对象、敌人职责和攻击提示必须不靠颜色也能辨认。

暂不优先：新手引导、长对话、剧情演出、最终美术、正式移动端、完整云服务、三地区量产和大型商业编辑器。
...
1. `PLATFORM-001`：建立真实 Windows 可见窗口与隔离 Web 玩家宿主，加载同一 `.tgdsbx`、薄运行状态和灰盒资源。
2. `TOOLS-004/005`：补编辑器新增/复制/删除、人物/敌人模板、波次/目标面板、二维摆放与启动/重载。
3. `GAME-005`：接入 Actor profile、职责、技能、两波战斗、目标推进与终点。
4. `GAME-006`：把死亡重试扩展为关卡、机关、波次、敌人和战斗状态的聚合恢复。
5. `UI/ART/QA`：补必要 HUD、攻击提示、可辨灰盒、Windows/Web/三浏览器证据和真人试玩。
...
| [`docs/02-版本规划与验收.md`](docs/02-版本规划与验收.md) | 当前团队、任务、Demo 阶段、工期和放行门 |
