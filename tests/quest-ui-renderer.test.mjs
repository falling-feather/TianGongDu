...
test("同一恢复 cue 为檐守与翻花各自呈现成对 offer/resume", async () => {
  const renderer = await loadRenderer();
  const recovery = (objectiveId, sourceName, sequence) => renderer.createViewModel(
    projectionEvent(renderer, {
      cueId: "ui.f1.training.recovery",
      objectiveId,
      sourceName,
      surfaceName: sourceName === "recovery_offer" ? "failure" : "gameplay",
      polarityName: "recovery",
      sequence
    })
  );
  const eavesguardOffer = recovery(
    "f1_objective_eavesguard_counter",
    "recovery_offer",
    "52"
  );
  const eavesguardResume = recovery(
    "f1_objective_eavesguard_counter",
    "recovery_resume",
    "53"
  );
  const flowerOffer = recovery(
    "f1_objective_flower_turn_counter",
    "recovery_offer",
    "54"
  );
  const flowerResume = recovery(
    "f1_objective_flower_turn_counter",
    "recovery_resume",
    "55"
  );
  assert.equal(eavesguardOffer.title, "回到沈砚训练点");
  assert.match(eavesguardOffer.prompt, /选线和取标保留/);
  assert.equal(
    eavesguardOffer.detail,
    "可以立即再试或离开；失败次数不会推进训练。"
  );
  assert.equal(eavesguardResume.title, "檐守训练已恢复");
  assert.match(eavesguardResume.prompt, /没有重复提交/);
  assert.match(eavesguardResume.detail, /不重放前置步骤/);
  assert.equal(flowerOffer.title, "回到沈砚训练点");
  assert.match(flowerOffer.prompt, /檐守证明、复盘、切式和越线保留/);
  assert.equal(
    flowerOffer.detail,
    "可以立即再试或离开；失败次数不会推进训练。"
  );
  assert.equal(flowerResume.title, "训练进度已恢复");
  assert.match(flowerResume.prompt, /只重建当前训练架/);
  assert.match(flowerResume.detail, /不重放对话，也不改线/);
  const playerCopy = [
    eavesguardOffer,
    eavesguardResume,
    flowerOffer,
    flowerResume
  ].map(({ title, prompt, detail }) => `${title}${prompt}${detail}`).join("");
  assert.doesNotMatch(playerCopy, /合格|计时|classification/);
});
