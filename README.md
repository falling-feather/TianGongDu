...
- 已实现确定性 `IQuestRuntime`、目标图快照/事件/校验和、内容驱动的场景交互、战斗信号、敌群结果与稳定选择记录；玩家可完成雨渡三项交互与沈砚双架势训练，失败重试后再以读招/闪避/反击清除两伞偶和纸鹭，选择雨棚路线，继而照见春/冬工痕、复核共享账簿并锁定春肋或冬肋调校，从而完成前 4 Beat。后续回程战、Boss 四阶段、双结算与奖励仍缺失，但均保留在既有 7 Beat 合同和 Reserved 接口边界内。
...
当前 Web 灰盒控制为：`WASD/方向键` 移动、`F` 交互、`Space` 纵跃、`J` 轻击、`K` 重击、`Shift` 守势、`C` 闪身、`1/2` 切换檐守/翻花，倒地后按 `R` 从当前 Bootstrap 安全点重试。它已经能直观看到《斗战神》第一指导的斜向纵深、深度排序与遮挡层次，以及前 4 Beat 的任务推进、姿态资源、命中/破势、读招闪避、资源恢复、失败重试、工痕调查和稳定调校选择，但还不是可连续游玩 1 小时的 Demo；后 3 Beat、正式鼠标/手柄动作适配与多安全点快照仍是缺项。
...
| 可玩纵切 | In Progress（前 4 Beat + 可操作战斗/调查灰盒） | F1 唯一流程、5 Cell/7 Beat 机器合同；代码绘制的 2.5D 斜向全景雨夜场景；雨渡交互、沈砚双架势训练、失败重试、三敌读招清场、路线选择、三证据工灯调查与双调校选择；资源恢复和任务进度保持 | 后 3 Beat 的回程战/捷径、复杂 AI/掉落、真实 5 Cell、Boss 四阶段、双结算和 60 分钟盲测 |
| 内容工具 | In Progress（Bootstrap） | 9 类模板注册表、工作台范围、F1 纵切、9 项场景交互、2 项战斗任务触发器、2 项敌群结果及 4 实体/9 能力战斗包的 JSON/Schema/确定性 C++ 生成 | 可用 Workbench、通用 ContentCore/baker、迁移、资源预览与错误定位 UI |
