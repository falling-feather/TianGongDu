...
内容激活使用独立 `EncounterActivationCommand`，以同一 Tick 和全局单调边界序列分别请求 `IEncounterDirector::activate_group` 与 `ICombatResolver::activate_group`；Director 更新归位点/接敌槽并只重置受影响的攻击 Runtime，Combat 按模式恢复/隔离 Actor。`SafePointRetryCommand` 及 `SafePointRetryReason::player_defeated` 只用于真正倒地的玩家，不再承载任务推进。失败重试先恢复当前 Beat 的入场 `replace`，再按 Definition 顺序重放当前任务图中已经完成 Objective 对应的激活，因此返程 `reinforce` 不会在重试后丢失。`CombatActorSnapshot` 以独立 `defeated` 位表达生命归零：`active=false && defeated=false` 是休眠/未选中，只有 `defeated=true && health=0` 才可计入击败任务；Active+Defeated、Defeated+非零生命均失败关闭。训练构件继续使用独立 Archetype，避免练习对手进入正式伞偶/纸鹭族群计数。

Stage Advance 和 Objective Complete 都可能由场景交互或 `CombatEvent` 产生。Web Shell 只能先记录待激活 Beat/Objective，再在交互解析后或战斗事件发布完成后的 Tick 安全点应用，禁止在 `ICombatEventSink::publish` 回调栈中重入修改 Combat Resolver。沈砚训练用两种单体教学方位、伞巷用 1/5 对向夹击与纵深纸鹭槽、返程用 0/3/6 三角入口组并在 Slot 5 追加纸鹭，共同证明同一内容边界可驱动完整替换与单 Actor 叠加增援。当前已实现的是 7 项固定 Definition 与“3→4”Bootstrap；动态队形、角色职责层、多个 Objective 会合、复合条件、多单位增援调度、刷出/退场演出、多令牌协同、导航避障和通用 Workbench 编队/时间线仍为 Missing/Reserved。
